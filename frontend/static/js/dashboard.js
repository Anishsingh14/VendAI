...
const DashboardPage = {
  render() {
    ...
    return pageShell('dashboard', `
      <div class="mb-8">
        <h1 class="text-2xl font-semibold text-slate-900">Welcome back, ${(user.name || 'there').split(' ')[0]}</h1>
        <p class="text-slate-500 text-sm mt-1">Here's what's happening with your vending machines.</p>
      </div>
      <div id="stats-row" class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8"></div>
    ...
    try {
      ...
      if (!machines.length) {
        ...
        ml.innerHTML = machines.map(m => `
          <div class="flex items-center justify-between px-5 py-4 hover:bg-slate-50 cursor-pointer transition-colors" onclick="Router.navigate('machine-detail',{id:'${m.id}',name:'${m.name}',location:'${m.location}'})">
